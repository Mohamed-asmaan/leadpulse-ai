...
export function OutreachHistoryList({ rows }: { rows: OutreachRow[] }) {
  ...
  return (
    <ul className="divide-y divide-border rounded-lg border border-border overflow-hidden">
      {rows.map((r) => (
        <li key={r.id} className="px-3 py-2.5 bg-card/50 text-sm space-y-1">
          ...
          <pre className="text-[11px] text-muted-foreground whitespace-pre-wrap max-h-24 overflow-auto">
            {r.message}
          </pre>
